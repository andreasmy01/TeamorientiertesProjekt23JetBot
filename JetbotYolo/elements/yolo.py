...
import cv2
import numpy as np
from JetbotYolo.models.experimental import attempt_load
from JetbotYolo.utils.general import non_max_suppression
from torch2trt import TRTModule

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

class OBJ_DETECTION():
    def __init__(self, model_path, classes, trt=False):
        self.classes = classes
        self.is_trt = trt
        if self.is_trt:
            self.yolo_model = TRTModule()
            self.yolo_model.load_state_dict(torch.load(model_path))
        else:  
    ...
    def detect(self,main_img):
        ...
        pred = self.yolo_model(img)[0] if self.is_trt else self.yolo_model(img, augment=False)[0]
